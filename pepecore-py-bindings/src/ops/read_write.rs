...
use pyo3::{Bound, PyAny, PyResult, Python, pyfunction, pyclass, pymethods, PyRef};
...
fn get_tile<T>(data:&[T],tile_size:usize,c:usize,w:usize,y_x:[usize;2])->Vec<T> where T: Clone{
    ...
    fn __next__<'py>(&mut self, py: Python<'py>) -> PyResult<Option<Bound<'py, PyAny>>> {
        ...
        let svec = py.detach(|| {
            let mut svec = match self.inner.pixel_type() {
                PixelType::U8 => {
                    let tile = get_tile::<u8>(self.inner.get_data::<u8>().unwrap(),self.tile_size,c.unwrap_or(1),w,self.x_y[self.pos]);
                    SVec::new(Shape::new(self.tile_size,self.tile_size,c),ImgData::U8(tile))

                }
                PixelType::F32 => {
                    let tile = get_tile::<f32>(self.inner.get_data::<f32>().unwrap(),self.tile_size,c.unwrap_or(1),w,self.x_y[self.pos]);
                    SVec::new(Shape::new(self.tile_size,self.tile_size,c),ImgData::F32(tile))

                }
                PixelType::U16 => {
                    let tile = get_tile::<u16>(self.inner.get_data::<u16>().unwrap(),self.tile_size,c.unwrap_or(1),w,self.x_y[self.pos]);
...
pub fn read_tiler(py: Python<'_>, path: String, color_mode: ColorMode, img_format: ImgFormat,tile_size:usize) -> PyResult<Option<Image>> {
    ...
    match result {
        Ok(img) => {
            ...
            Ok(Some(Image { inner: img,format:img_format, tile_size,x_y:tiles,color:true,pos:0})) },
