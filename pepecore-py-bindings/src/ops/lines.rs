...
use pyo3::{
    exceptions::PyTypeError, Bound, PyAny, PyRef, PyResult, Python,
    pyclass, pymethods, pyfunction
};
use pepecore::{Point, Line, draw_lines};
    ...
    fn one<'py>(o: &Bound<'py, PyAny>) -> PyResult<Line> {
        ...
        if let Ok(bd) = o.extract::<PyRef<'py, PyBezier>>() {
            ...
            return Ok(Line::Bezier(
                b.p0.into(),b.p1.into(),b.p2.into(),b.p3.into(),b.step
            ));
        }
