...
pub fn jpeg_compress(img:&mut SVec,quality:u8,qt:&QuantizationTableType,yuv:&YCbCrRatio){
    ...
    if c ==3{
        ...
        jpeg_compress_one_ch(&mut u,h/ver,w/hor,&QuantizationTable::new_with_quality(qt,quality,false));
        jpeg_compress_one_ch(&mut v,h/ver,w/hor,&QuantizationTable::new_with_quality(qt,quality,false));
        ycbcr_to_data(&y,&u,&v,h,w,ver,hor,img.get_mut_ptr().unwrap());
    }
    else if c==1 {
        jpeg_compress_one_ch(img.get_data_mut().unwrap(),h,w,&QuantizationTable::new_with_quality(qt,quality,true));
    }
